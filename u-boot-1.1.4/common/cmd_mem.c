...
#define MAGIC 0xea000006

int do_cksum_cmp (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
  int rcode;
  header *phead;
  ulong cksum2;
  ...
  if(phead->magic != MAGIC) {
    printf("Bad magic number 0x%08.8x(0x%08.8x)\n", phead->magic, MAGIC);
    return(1);
  }
#ifdef DO_FULL_CRC_CHECK
  // compute crc32
  crc= crc32(0, paddr1 + (sizeof (header)), phead->pcksum2 - sizeof (header));

  // check if the crc32 is correct
  if (crc != phead->cksum) {
    printf("Bad checksum 0x%08.8x(0x%08.8x)\n", crc, phead->cksum);
    return(1);
  }
#endif
  // check if the header and trailer checksum are identical
  cksum2= *(unsigned int *)&paddr1[phead->pcksum2+12];
  if(cksum2 != phead->cksum) {
    rcode= 1;
    printf("bad checksum header 0x%08.8x, trailer 0x%08.8x\n", phead->cksum,
	   cksum2);
  } else {
...
U_BOOT_CMD(
	cksum,    2,     1,     do_cksum_cmp,
	"cksum     - check crc32 on image and compare to header\n",
	"addr\n    - address of image\n"
);
