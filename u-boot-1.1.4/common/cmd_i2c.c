...
#define I2C_K                     (1000)
#define LCM_SD1602X_I2C_CLK          (10 * 1000)


#include <asm/arch-str8131/star_i2c.h>
#include <asm/arch-str8131/star_misc.h>
#include <asm/arch-str8131/star_powermgt.h>
...
void Hal_I2c_Initialize(I2C_OBJECT_T *pI2c_Object)
{
    ...
    /*
     * Note I2C is NOT enabled after this function is invoked!!
     */
    I2C_CONTROLLER_REG = pI2c_Object->control_reg;

    I2C_TIME_OUT_REG = pI2c_Object->time_out_reg;

    I2C_INTERRUPT_ENABLE_REG = pI2c_Object->interrupt_config;

#if 0
    if (pI2c_Object->interrupt_config)
    {
        /*
         * Register I2C's interrupt ISR
         */
        Hal_Intc_Register_Interrupt(&pI2c_Object->intc_obj);        
    }
#endif
...
int lcm_init()
{
	I2C_OBJECT_T i2c_object;
    	u32 i2c_clkdiv;
	u32 i2c_apb_clk;
	u8 status;

#ifdef BUILD_FOR_RTL
    i2c_apb_clk = (50 * I2C_K * I2C_K);
#elif defined(BUILD_FOR_ASIC)
    i2c_apb_clk = Hal_Pwrmgt_Inquiry_System_APB_Clock();
#else
    i2c_apb_clk = (13 * I2C_K * I2C_K);
#endif

    //printf("\nI2C: APB clock = %dMHz\n", i2c_apb_clk);

    /*
     * I2C clock frequency = PCLK / 2*(CLKDIV + 1)
     */
    i2c_clkdiv = ((i2c_apb_clk * 1000000) / (2 * LCM_SD1602X_I2C_CLK)) - 1;


    /*
     * Configure I2C Master: disable I2C, stop I2C bus action
     */
    i2c_object.control_reg = (0 << 6) |     /* Set I2C Bus Idle */
                             (0 << 24) |    /* Disable I2C Data Swap */
                             (0 << 31);     /* Disable I2C */

    i2c_object.time_out_reg = ((i2c_clkdiv & 0x1FF) << 8) |  /* Set I2C clock */
                              (1 << 7) |  /* Enable time-out check */
                              (0x40);     /* Set time-out value */
#if 0
    /*
     * Register I2C interrupt sources
     */
    i2c_object.intc_obj.interrupt_index = INTC_I2C_BIT_INDEX;
    
    i2c_object.intc_obj.interrupt_type = INTC_IRQ_INTERRUPT;
    
    i2c_object.intc_obj.trigger_mode = INTC_LEVEL_TRIGGER;
    
    i2c_object.intc_obj.trigger_level = INTC_ACTIVE_LOW;

    i2c_object.intc_obj.priority_level = 0;
    
    i2c_object.intc_obj.isr_handler_addr = (u_int32)I2C_IRQ_Handler;

    i2c_object.intc_obj.next = (INTC_OBJECT_T *)(0);

    // Enable I2C's interrupt sources
    i2c_object.interrupt_config = (I2C_BUS_ERROR_FLAG | I2C_ACTION_DONE_FLAG);

#endif
    // Initialize I2C's setting
	Hal_I2c_Initialize(&i2c_object);


	HAL_I2C_ENABLE_I2C();
        //printf("\nI2C: SD1602X initialization!!\n");
	//Initialize SD1602X
	if (!I2c_Lcm_Sd1602x_Init())
	{
        	printf("\nI2C: SD1602X initialization failed!!\n");
        	return status;
	}
...
//#define BUILD_FOR_ASIC

// test i2c lcm device
int do_i2c_lcm(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	} // end if (argc >= 2 && strcmp(argv[1], "init")==0)
#endif
	...
	if (argc >= 2) {
		...
		switch (argv[1][0])
		{
			case 'r':
			{
				...
				//printf("read data:\n");
				I2c_Lcm_Sd1602x_Read_Data_Command(&read_data);
				printf("read_data: %x\n", read_data);
				break;
			}
			case 'p':
			...
			case 'w':
			{
				...
				//printf("write data:\n");
				//i2c_lcm_sd1602x_display_put_char(argv[2][0]+char_offset);
				i2c_lcm_sd1602x_display_put_char(argv[2][0]);
				break;
			}
		}
	...
	// command example : i2c_lcm 1 A
	if (argc >= 3)
	{
		u8 ddram_data = 0xb1; // char '1' (0x31)
		const u8 char_offset='1'-0xb1;

		// SD1602X display test
		if (!I2c_Lcm_Sd1602x_Display_Test(simple_strtoul(argv[1],0,10), argv[2][0]+char_offset))
		{
			printf("\nI2C: SD1602X display test failed!!\n");
        		return status;
		}
	}
#endif
	//printf("\nI2C: SD1602X display test OK!!\n");
