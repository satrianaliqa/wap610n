...
int do_led (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	u32 gpio_data;
	int i;
	int led =0, state=0, duration =5; //Default 
	/* Read Led index, state (on/off/blink) and duration if exist*/
	if (argc > 1)
	{
		...
		if (argc > 3)
		{
			duration = simple_strtoul(argv[3], NULL, 10);
		}
	}
    if (led < 0 || led > 20)
	{
	...
	switch (state)
	{
		case 0:// LED OFF
			...
			gpio_data |= (1 <<led); 
    		GPIO_GPIO_DATA_OUT_REG = gpio_data;
			break;
		case 1: // LED ON
     		gpio_data = GPIO_GPIO_DATA_IN_REG;
			gpio_data &= ~(1 <<led); 
    		GPIO_GPIO_DATA_OUT_REG = gpio_data;
			break;
		case 2: //Blink ("Duration" or 5 sec.).
			for (i=0; i< (2*duration);i++)  
			{
	    	    /* Flash a led*/
    			gpio_data = GPIO_GPIO_DATA_IN_REG;
				gpio_data &= ~(1 <<led); // LED ON
    			GPIO_GPIO_DATA_OUT_REG = gpio_data;
				udelay(250*1000); // 250 msec
    			gpio_data = GPIO_GPIO_DATA_IN_REG;
				gpio_data |= (1 <<led); // LED OFF
    			GPIO_GPIO_DATA_OUT_REG = gpio_data;
				udelay(250*1000); // 250 msec
			}
			break;
...
U_BOOT_CMD(
	led,    4,    1,    do_led,
 	"led    - Set led on, off or blink. \n",
	"\n     - Set Led index 0 to Off state (Default settings)\n"
	"led <gpio index> <0 (Off) / 1 (On) / 2 (Blink)> [duration]\n    - Set the gpio (led) to On, Off or Blinking state. \nIf blinking set add also the duration (in seconds)\n"
);
