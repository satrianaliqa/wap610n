...
/*
do_check_restore_default ()-
  Check if Restore Default button is pressed
  The button should be pressed for at least 5 seconds
  Input: Restore Default gpio index
*/
int do_check_restore_default ( cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	u32 gpio_data;
	int rcode,i;
	int gpio; //gpio index of Restore Default button
	/* Read Restore Default gpio*/
	gpio = simple_strtoul(argv[1], NULL, 10);
    if (gpio < 0 || gpio > 20)
	{
		puts ("Error: Wrong input. This GPIO pin is not available\n");
		return 1;
	}
    // Direction bit for the LED and Res. Default pins
    gpio_data = GPIO_PIN_DIR_REG;
    gpio_data &= ~(1 << gpio); 
    GPIO_PIN_DIR_REG = gpio_data;
    for (i=0; i<5;i++) //check button status (pressed or released) for 5 sec. 
	{
		// Read GPIO
    	gpio_data = GPIO_GPIO_DATA_IN_REG;
    	/* Mask the Restore Default bit
       	   rcode=0 (restore default button is pushed) or 
		   rcode=1 (button is not pushed)
		*/ 
		rcode = ((gpio_data >> gpio)&0x1);
		if (rcode)
		{
			return rcode;
		}
...
U_BOOT_CMD(
	check_restore_default,    2,    1,    do_check_restore_default,
	"check_restore_default  - Check if Restore Defaut is pressed\n",
	"\n    - Check if Restore Defaut is pressed (GPIO 13)\n"
	"check_restore_default N\n    - Check if Restore Defaut is pressed. N=Define the gpio pin number of Restore Default button\n"
);
