...
#ifdef CONFIG_UMEDIA
#define	INIT_PORT0_PHY rtl_8201cp_init(0);
#define	INIT_PORT1_PHY rtl_8201cp_init(1);
//U-MEDIA, PHY ADDR for RTL8201CP is 1
#define PORT0_LINK_UP std_phy_power_down(1, 0);
#define PORT0_LINK_DOWN std_phy_power_down(1, 1);
#define PORT1_LINK_UP  std_phy_power_down(1,0);
#define PORT1_LINK_DOWN std_phy_power_down(1,1);
#endif
...
#if defined (CONFIG_DORADO2) || defined (CONFIG_GPB239M)		// robin 080117
#define	INIT_PORT0_PHY star_gsw_config_port0_VSC7385();
...
static int star_gsw_config_VSC8601(u8 mac_port, u8 phy_addr)
{
	...
	#if ((!defined CONFIG_GPB239M) &&(!defined CONFIG_GPB239S))
        // enable PHY's AN
	mac_port_config |= (0x1 << 7);
	#else
	// disable PHY's AN
...
#else
#if (defined CONFIG_LEO) || (defined CONFIG_GPB239S)
...
#ifdef CONFIG_UMEDIA
int rtl_8201cp_init (int port)
{
	if (port == 0)	 {// port 0
		printf("init RTL8201CP\n");
...
#if defined(CONFIG_DORADO) || defined(CONFIG_DORADO2) || defined (CONFIG_GPB239M)
static int star_gsw_config_port0_VSC7385(void)
{
	u32 mac_port_config;
	int i;
