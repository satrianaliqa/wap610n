 ...
 */
	...
	if (variables == -1) {
		ep->variables[vid] = symOpen(67) + EJ_OFFSET;
...
static int parseStmt(ej_t *ep, int state, int flags)
{
	...
	int			ejVarType;
	char_t		MTvalue[MT_MAX_PARAM_VALUE_LENGTH+1];
				...
				if (flags & FLAGS_EXE) {
					if ( state == STATE_DEC ) {
						...
						if (ejVarType > 0) {
							...
							if (!MT_Set_Param(identifier, ep->result, MT_DONT_SET_FIRST_VALUE))
							{
 ...
 */
				...
				if (state == STATE_DEC) {
					...
					if ( flags & FLAGS_EXE ) {
						...
						if (ejGetVar(ep->eid, identifier, &value) < 0) 
						{
							if (MT_Get_Param(identifier,MTvalue,MT_MAX_PARAM_VALUE_LENGTH-1))
							{
								//ejSetLocalVar(ep->eid, identifier, MTvalue[0] ? MTvalue : NULL);
								value = MTvalue;
							}
						}
 ...
 */
			...
			setString(B_L, &ep->result, MTretValue);
