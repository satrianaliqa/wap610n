...
{
	debug_printf("websUrlHandlerDefine: Into websUrlHandlerDefine function.\n");
	debug_printf("websUrlHandlerDefine: urlPrefix=%s, webDir=%s, arg=%d, flags=%d\n", urlPrefix, webDir, arg, flags);
	websUrlHandlerType	*sp;
	int					len;
 ...
 */
	qsort(websUrlHandler, websUrlHandlerMax, sizeof(websUrlHandlerType), 
		websUrlHandlerSort);
	debug_printf("websUrlHandlerDefine: Exit websUrlHandlerDefine function.\n");
	return 0;
}
 ...
 */
	...
	for (i = 0; i < websUrlHandlerMax; i++) {
		...
		if (sp->handler && gstrncmp(sp->urlPrefix, wp->path, sp->len) == 0) {
			if (first) {
				//printf("wp->flags & WEBS_HNAP_FW_UPGRADE = %0xx\n", (wp->flags & WEBS_HNAP_FW_UPGRADE)); //Ricky Trace
				if(!(wp->flags & WEBS_HNAP_FW_UPGRADE)){// Add for HNAP Firmware Upgrade feature - Ricky Cao
					websSetEnv(wp);
				}
				first = 0;
			}
