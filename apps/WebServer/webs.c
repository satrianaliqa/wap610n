...
static void	 extractUploadedFileContent(webs_t wp);
...
//Add for WET610N - Ricky Cao on 2008/09/03
//Use device name to instead of the original realm
void websInitRealm(void)
{
	char DeviceName[MT_MAX_PARAM_VALUE_LENGTH]={0};
	MT_Get_Param("HostName", DeviceName, sizeof(DeviceName));
	printf("DeviceName for realm = %s (len = %d)\n", DeviceName, strlen(DeviceName));
	//printf("websRealm -1 = %s\n", websRealm);
	if(strcmp(DeviceName, websRealm)){
		if(strlen(DeviceName)>=64){
			strcpy(websRealm, "GoAhead");
		}else{
			memset(websRealm, 0, 64);
			memcpy(websRealm, DeviceName, strlen(DeviceName));
		}
	}
	//printf("websRealm -2 = %s\n", websRealm);
}
//Ricky Cao on 2008/09/03
 ...
 */
	websMime = symOpen(WEBS_SYM_INIT_4099);
	a_assert(websMime >= 0);
 ...
 */
	...
	trace(0, T("webs: Listening for HTTP requests at address %s port %d\n"),
		websIpaddrUrl,websPort);
#ifdef WIN 
    {
        char url[128];
        sprintf(url,"http://%s",websIpaddrUrl);
...
#endif
            if (strcmp(wp->url,"/goform/upldImage")==0 || wp->flags & WEBS_HNAP_FW_UPGRADE) //Tom.Hung 2009-7-29, add HNAP f/w upgrade check
            {
								printf("getFlashSize: %d\n", getFlashSize());
								printf("wp->clen: %d\n", wp->clen);
                /* allocate a new buffer with a good size */
                char * buffer = NULL;
                ...
                {
                    char tmp[MAX_LINE];
                    /* parsing will fail, and an error will be shown */
                    if (wp->FileContentLen == 0)
                    		//Tom.Hung 2009-7-21, replace IMAGE_MAX_SIZE to getFlashSize()
                        //wp->FileContentLen = (wp->clen > IMAGE_MAX_SIZE) ? UPLOAD_FILE_TO_BIG : UPLOAD_NO_MEMORY;
                        wp->FileContentLen = (wp->clen > getFlashSize()) ? UPLOAD_FILE_TO_BIG : UPLOAD_NO_MEMORY;
                        //Tom.Hung 2009-7-21
                    /* just read all data */
                    wp->clen -= nbytes;
                    while (wp->clen >0)
                ...
                } else
                {
                    if (wp->postData)
                    {
                        memcpy(buffer,wp->postData, wp->lenPostData);
                        bfree(B_L,wp->postData);
                    }
                    memcpy(buffer+wp->lenPostData, text, nbytes);
                    wp->lenPostData+=nbytes;
                    wp->postData = buffer;
...
*/
                    wp->postData = brealloc(B_L, wp->postData,(wp->lenPostData + nbytes +	1) * sizeof(char_t));
 ...
 */
				...
				//Add for pass the test of TestDevice.App 11.0.8303.2 - Ricky Cao on Nov. 3 2008
				if(wp->state == WEBS_POST && len==0){
					websError(wp, 500, T("INCORRECT HTTP CONTENT"));
				}
			}
/*
 ...
 */
	...
	else if (gstrcmp(ext, T(".js")) == 0) { //Jacky.Yang 8-Feb-2009, for support *.js can use asp parsing.
		wp->flags |= WEBS_ASP;
	}
	bfree(B_L, buf);
...
static void websParseRequest(webs_t wp)
{
	...
	int b64Decode_len=0; //Add for HNAP - Ricky Cao
 ...
 */
			...
			if(strlen(value)==0){ //This is add for Linksys Direct TV team report bug when information for authorization is NULL, the web server will crash -Ricky Cao on Nov. 21 2008
				printf("WARNING: The value for authorization header is NULL..\n");
 ...
 */
			...
			}else{ //Add by Ricky Cao on Nov. 21 2008
/*
 ...
 */
				...
				}//Add by Ricky Cao on Nov. 21 2008
/*
 ...
 */
		...
		//Add for HNAP firmware upgrade - Ricky Cao
		else if (gstrcmp(key, T("soapaction")) == 0) {
			if(gstrstr(value, T("FirmwareUpload"))){
				printf("This is a HNAP Firmware Upload request !!\n");
				wp->flags |= WEBS_HNAP_FW_UPGRADE;
			}
		}
		// - Ricky Cao
	}
...
#ifdef WIN
    websSetVar(wp, T("SERVER_TYPE"),T("WIN"));
#else
    websSetVar(wp, T("SERVER_TYPE"),T("LINUX"));
#endif
    /* multipart */
...
/* 
	...
	Due to the call to websDecodeUrl in websUrlParse (url.c) the wp->query string is allready decoded,
	meaning that if there was a & sign in the paramters passed to the web, they could mess up the query
	string.

	The solution is simply use the url instead of the wp-query to tokenize the parameters at this point.


	startOfQuery = strchr(wp->url,'?');
	if (startOfQuery)
		startOfQuery++;

	wp->decodedQuery = bstrdup(B_L, strchr(wp->url,'?')+1);

	In addition, we need to make sure that wp->query contains the query before the decoding, and wp-decodedQuery
	contains the decoded query.
	...
	while (keyword != NULL) {
		if ((value = gstrchr(keyword, '=')) != NULL) {
			*value++ = '\0';
			...
			//Jacky.Yang 11-Jul-2008, solved access contorl will set fail value.
			//MT_Set_Param(keyword, value, MT_DONT_SET_FIRST_VALUE); // Update the parameter value in our temporary array
		} else {
			value = T("");
		}
 ...
 */
	wp->cgiVars = symOpen(WEBS_SYM_INIT_4099);
                        ...
                        if (endPart != NULL) {
                            if ((tmpStartStr = gstrstr(buf, "filename=")) != NULL) {
                                ...
                                if (!firstUploadFile) {
                                    ...
                                    wp->FileContentLen = (int)endPart - (int)part - 2; /* exclude the ending \r\n */
                                    wp->FileContent= part;
                                }
                                ...
                                if ((tmpEndStr = gstrchr(&tmpStartStr[17], '\"')) != NULL) {
                                    if ((tmpValLenByte = ((int)tmpEndStr - (int)&tmpStartStr[17])) > 0) {
                                        ...
                                        if ((tmpValLenByte = ((int)endPart - (int)part - 2)) > 0) {
                                            tmpValLenByte = (tmpValLenByte < (MAX_LINE-(int)strlen(tmpNameStr))) ? tmpValLenByte : (MAX_LINE-strlen(tmpNameStr)-1);
