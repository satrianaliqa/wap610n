...
int websFormDefine(char_t *name, void (*fn)(webs_t wp, char_t *path, 
	char_t *query))
{
	debug_printf("websFormDefine: Into websFormDefine function.\n");
	debug_printf("websFormDefine: name:%s\n", name);
	a_assert(name && *name);
	a_assert(fn);
	...
	debug_printf("websFormDefine: Exit websFormDefine function.\n");
	return 0;
}
...
void websFormOpen()
{
	formSymtab = symOpen(WEBS_SYM_INIT_521);
}
