...
int websVirtualDirHandler(webs_t wp, char_t *urlPrefix, char_t *webDir, int arg,
						char_t *url, char_t *path, char_t *query)
{
	...
	if (webDir)
	{
		...
		wp->dir=bstrdup(B_L, webDir);
	}


	return websDefaultHandler(wp,"","",arg,url+(strlen(urlPrefix)),path+(strlen(urlPrefix)),query);
}
...
int websDefaultHandler(webs_t wp, char_t *urlPrefix, char_t *webDir, int arg,
						char_t *url, char_t *path, char_t *query)
{
	debug_printf("websDefaultHandler: Into websDefaultHandler function. (We will not display Exit message.)\n");
	debug_printf("websDefaultHandler: urlPrefix=%s, webDir=%s, arg=%d, url=%s, path=%s, query=%s\n", urlPrefix, webDir, arg, url, path, query);
	websStatType	sbuf;
	...
	debug_printf("websDefaultHandler: Validate the URL and ensure that \"..\"s don't give access to unwanted files\n");
/*
 ...
 */
	...
	if (websValidateUrl(wp, path) < 0) 
   {
	...
	debug_printf("websDefaultHandler: If the file is a directory, redirect using the nominated default page\n");
/*
 ...
 */
	...
	debug_printf("websDefaultHandler: Output the normal HTTP response header\n");
/*
 ...
 */
	...
	debug_printf("websDefaultHandler: By license terms the following line of code must not be modified.\n");
/*
 ...
 */
	...
	debug_printf("websDefaultHandler: Evaluate ASP requests\n");
/*
...
#else
	debug_printf("websDefaultHandler: For normal web documents, return the data via background write\n");
/*
