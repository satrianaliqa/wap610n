 ...
 */
#define UM_XOR_ENCRYPT	T("s#$5wert#$WEAT768d@#4a#$%215$T^QWRtr5TrgsDF$")
 ...
 */
	if ((row = dbSearchStr(didUM, UM_USER_TABLENAME, UM_NAME, user, 0,DB_SEARCH_START_BEGIN)) >= 0) {
...
bool_t umUserExists(char_t *user)
{
	...
	if (dbSearchStr(didUM, UM_USER_TABLENAME, UM_NAME, user, 0,DB_SEARCH_START_BEGIN) >= 0) {
		return TRUE;
	} else {
		return FALSE;
	}
...
char_t *umGetUserPassword(char_t *user)
{
	...
	password = NULL;
	row = dbSearchStr(didUM, UM_USER_TABLENAME, UM_NAME, user, 0,DB_SEARCH_START_BEGIN);
 ...
 */
	if ((row = dbSearchStr(didUM, UM_GROUP_TABLENAME, UM_NAME, group, 0,DB_SEARCH_START_BEGIN)) < 0) {
		return UM_ERR_NOT_FOUND;
	}
...
bool_t umGroupExists(char_t *group)
{
	...
	if (dbSearchStr(didUM, UM_GROUP_TABLENAME, UM_NAME, group, 0,DB_SEARCH_START_BEGIN) >= 0) {
		return TRUE;
	} else {
		return FALSE;
	}
 ...
 */
	if (dbSearchStr(didUM, UM_USER_TABLENAME, UM_GROUP, group, 0,DB_SEARCH_START_BEGIN) >= 0) {
		return TRUE;
	} 
 ...
 */
	if (dbSearchStr(didUM, UM_ACCESS_TABLENAME, UM_GROUP, group, 0,DB_SEARCH_START_BEGIN) >= 0) {
		return TRUE;
	} 
...
accessMeth_t umGetGroupAccessMethod(char_t *group)
{
	...
	a_assert(group && *group);
	row = dbSearchStr(didUM, UM_GROUP_TABLENAME, UM_NAME, group, 0,DB_SEARCH_START_BEGIN);
 ...
 */
	if ((row = dbSearchStr(didUM, UM_ACCESS_TABLENAME, UM_NAME, url, 0,DB_SEARCH_START_BEGIN)) < 0) {
		return UM_ERR_NOT_FOUND;
	}
...
bool_t	umAccessLimitExists(char_t *url)
{
	...
	if (dbSearchStr(didUM, UM_ACCESS_TABLENAME, UM_NAME, url, 0,DB_SEARCH_START_BEGIN) < 0) {
		return FALSE;
	} else {
		return TRUE;
	}
...
accessMeth_t umGetAccessLimitMethod(char_t *url)
{
	...
	am = (int) AM_INVALID;
	row = dbSearchStr(didUM, UM_ACCESS_TABLENAME, UM_NAME, url, 0,DB_SEARCH_START_BEGIN);
