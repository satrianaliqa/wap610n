...
int send_renew(uint32_t xid, uint32_t server, uint32_t ciaddr)
{
	...
	/* ArnonM  HACK: Send the packet with the server ID option set.
	 * This improves behavior when interoping with the WRT610, 
	 * because Cybertan added a hack to send broadcast when receiving this option.  
	 * Also set requested IP option, or else non-cybertan udhcpd servers will drop the request */
	//Jacky.Yang 17-Jun-2009, drop it, try to get the same behavior wirh previous WET610N f/w.
