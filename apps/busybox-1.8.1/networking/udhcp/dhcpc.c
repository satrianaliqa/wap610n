...
/* Add by U-Media Ricky Cao on Dec. 05 2008 */
#include<sys/stat.h>
#include<unistd.h>
#include <errno.h>
		...
		if (retval < 0) {
			...
			case INIT_SELECTING:
				if (packet_num < client_config.retries) {
					...
					/*Because WPS is running, so we don't want WET610N to get IP from DHCP server - Ricky Cao on Dec. 05 2008 */
					result = stat("/var/wpsRunning", &statBuf);
					if(result==-1 && errno==2){
						/* send discover packet */
						send_discover(xid, requested_ip); /* broadcast */
					}else{
						system("echo \"INFO: WPS is running, so doesn't get IP from DHCP server...\" > /dev/console");
					}
					/*Revised by Ricky Cao on Dec. 05 2008 */
					...
					/* wait to try again */
					packet_num = 0;
					timeout = now + 30; //Revised from 60 to 30 by Ricky Cao on 2008/09/24
				}
				break;
			...
			case REQUESTING:
				if (packet_num < client_config.retries) {
					...
					if (state == RENEW_REQUESTED){
						//Add by Ricky Cao on Nov. 26 2008
						//For require the same IP from DHCP Server
						//Jacky.Yang 26-Jun-2009, try to re-new
						//perform_renew();
						//send_release(server_addr, requested_ip); /* unicast */
						//system("ifconfig br0 0.0.0.0");
						//by Ricky Cao on Nov. 26 2008
						udhcp_run_script(NULL, "deconfig");
					}
					state = INIT_SELECTING;
					timeout = now;
