...
create_links_in_tmp ()
{
	...
	if [ $NETWORK_TYPE = 0 ]; then ln -s $CONFIGS_PATH/wpa_supplicant0.conf   /tmp/config.conf; fi
...
insmod_driver
#4 Use SendKey form a driver (start 2.3.10)
...
if [ ! -e /tmp/init_failed ]; then set_wls_bridging; fi	
#################################################################
