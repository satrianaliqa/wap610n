	echo "Configuring Linux QoS Queues (modified 802.1Q mapping)"
	...
	tc filter add dev wlan0 parent 1:0 protocol ip prio 2 u32 match ip tos 160 0xff flowid 1:1
	tc filter add dev wlan0 parent 1:0 protocol ip prio 2 u32 match ip tos 128 0xff flowid 1:2
	tc filter add dev wlan0 parent 1:0 protocol ip prio 3 u32 match ip tos 96 0xff flowid 1:2
	tc filter add dev wlan0 parent 1:0 protocol ip prio 4 u32 match ip tos 64 0xff flowid 1:2
