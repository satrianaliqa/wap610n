...
static inline unsigned int str8131_read_timer_counter(void)
{
	return TIMER1_COUNTER_REG;
}

static inline unsigned int str8131_read_timer_interrupt_status(void)
{
	return TIMER1_TIMER2_INTERRUPT_STATUS_REG;
}

static inline void str8131_clear_timer_interrupt_status(unsigned int irq)
{
	TIMER1_TIMER2_INTERRUPT_STATUS_REG &= ~(1 << TIMER1_OVERFLOW_INTERRUPT_BIT_INDEX);
}

static void str8131_setup_timer(unsigned int counter_value)
{
...
static void str8131_timer_enable(void)
{
	unsigned long control_value;
...
#if 0
static void str8131_timer_disable(void)
{
	unsigned long control_value;
...
#endif /* Disable function str8100_timer_disable */

/*
 * Returns number of us since last clock interrupt.  Note that interrupts
 * will have been disabled by do_gettimeoffset()
 */
static unsigned long str8131_gettimeoffset(void)
{
	 ...
	 */
	ticks2 = str8131_read_timer_counter();
	do {
		ticks1 = ticks2;
		interrupt_status = str8131_read_timer_interrupt_status();
		ticks2 = str8131_read_timer_counter();
	} while (ticks2 > ticks1);
 ...
 */
static irqreturn_t
str8131_timer_interrupt(int irq, void *dev_id, struct pt_regs *regs)
{
	write_seqlock(&xtime_lock);

#ifndef CONFIG_VIC_INTERRUPT
	str8131_clear_timer_interrupt_status((unsigned int)irq);
#endif
	timer_tick(regs);
...
static struct irqaction str8131_timer_irq = {
	.name		= "STR8131 Timer Tick",
	.flags		= SA_INTERRUPT | SA_TIMER,
	.handler	= str8131_timer_interrupt,
};

/*
 * Set up timer interrupt, and return the current time in seconds.
 */
static void __init str8131_timer_init(void)
{
	/*
	 ...
	 */
	str8131_setup_timer(timer_counter_value);

	/*
	 * Make irqs happen for the system timer
	 */
	setup_irq(INTC_TIMER1_BIT_INDEX, &str8131_timer_irq);

	str8131_timer_enable();
}

struct sys_timer str8131_timer = {
	.init		= str8131_timer_init,
	.offset		= str8131_gettimeoffset,
};
