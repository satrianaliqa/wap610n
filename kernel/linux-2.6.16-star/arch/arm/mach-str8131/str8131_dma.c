...
//ArnonM: Removed DMA_TEST
//#define STR8131_DMA_TEST
...
static int __init str8131_dma_init(void)
{
	int retval;
	...
	if (retval == 0) {
		...
		(void)str8131_dmacopy_busywait_test();
		(void)str8131_dmacopy_llp_test();
	}
#endif
	return retval;
}

static void __exit str8131_dma_exit(void)
{
	return;
}

module_init(str8131_dma_init);
module_exit(str8131_dma_exit);
...
static int str8131_dmacopy_busywait_test(void)
{
	...
	printk("STR8131 DMA Busy Wait Testing...\n");
	...
	printk("STR8131 DMA Busy Wait Testing end\n");
	if (memcmp(src, dst, MEMSIZE) == 0) {
		printk("STR8131 DMA Busy Wait Testing success\n");
		printk("STR8131 DMA Busy Wait Testing speed: %dMB/s\n", (u32)((MEMSIZE_K) * HZ/(jiffies - sjiffies)));
	} else {
		printk("STR8131 DMA Busy Wait Testing failed\n");
		for (i = 0; i < MEMSIZE; i++) {
			if (src[i] != dst[i]) {
				err_cnt++;
			}
	...
	if (err) {
		...
		for (i = 0; i < NUM_DMA_VEC; i++) {
			if (memcmp((src_vec[idx] + i * NUM_BYTES_PER_VEC), (dst_vec[idx] + i * NUM_BYTES_PER_VEC), NUM_BYTES_PER_VEC) == 0) {
				...
				printk("STR8131 DMA Testing error started at offset: %04d\n", i * NUM_BYTES_PER_VEC);
