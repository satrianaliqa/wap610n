...
static struct map_desc str9100_std_desc[] __initdata = {
	{
	//Jacky.Yang 20-May-2009, Begin - Power Management patch
		.virtual = SYSVA_FLASH_BASE_ADDR,
		.pfn = __phys_to_pfn(SYSPA_FLASH_BASE_ADDR),
		.length = SZ_8M,
		.type = MT_DEVICE
	}, {
	//Jacky.Yang 20-May-2009, End - Power Management patch
...
extern unsigned long __dspad_fix_sz_begin;
extern unsigned long __dspad_fix_sz_end;
extern int str9100_enable_dspad(unsigned long base_addr, unsigned long size);
#endif
u32 CPU_clock;
u32 AHB_clock;
u32 APB_clock;
...
void __init str9100_early_init(void)
{
	...
	//Jacky.Yang 20-May-2009, Begin - disable usb clock.
	HAL_PWRMGT_DISABLE_USB_HCLK();
	//Jacky.Yang 20-May-2009, End - disable usb clock.
#ifdef CONFIG_CPU_ISPAD_ENABLE
	str9100_enable_ispad((unsigned long)&__ispad_begin);
#endif
#ifdef CONFIG_CPU_DSPAD_ENABLE
        str9100_enable_dspad(DSPAD_BASE_VIRT,
		(u8 *)&__dspad_fix_sz_end - (u8 *)&__dspad_fix_sz_begin);
#endif
}
/* ######################################################################### */
