...
#ifdef DEBUG
static void debug_print_info(u32 lvl, char* func_name,char* more,struct snd_pcm_substream *substream){
...
#define MYPRINT(head,xx,fmt,args...) printk(#xx"="fmt" ",head->xx,args)
	MYPRINT(runtime,rate,"%d",NULL);

	MYPRINT(runtime,channels,"%d",NULL);
	MYPRINT(runtime,buffer_size,"%d(%#x)",runtime->buffer_size);
	MYPRINT(runtime,period_size,"%d(%#x)",runtime->period_size);
	MYPRINT(runtime,frame_bits,"%d",NULL);
	MYPRINT(runtime,sample_bits,"%d",NULL);
printk("\n           ");
	MYPRINT(runtime,dma_area,"%#.8x",NULL);
	MYPRINT(runtime,dma_addr,"%#.8x",NULL);
	MYPRINT(runtime,dma_bytes,"%d(%#x)",runtime->dma_bytes);
printk("\n           ");
	MYPRINT(runtime,dma_buffer_p,"%#p",NULL);
	if(runtime->dma_buffer_p){
		MYPRINT(runtime->dma_buffer_p,area,"%#p",NULL);
		MYPRINT(runtime->dma_buffer_p,addr,"%#p",NULL);
		MYPRINT(runtime->dma_buffer_p,bytes,"%#p",NULL);
		MYPRINT(runtime->dma_buffer_p,private_data,"%#p",NULL);
	}
printk("\n           param: ");
	MYPRINT(runtime,start_threshold,"%d",NULL);
	MYPRINT(runtime,stop_threshold,"%d",NULL);
	MYPRINT(runtime,xfer_align,"%d",NULL);
	MYPRINT(runtime,boundary,"%d",NULL);
printk("\n                  ");
	MYPRINT(runtime,silence_threshold,"%d",NULL);
	MYPRINT(runtime,silence_size,"%d",NULL);
	MYPRINT(runtime,silence_start,"%d",NULL);
	MYPRINT(runtime,silence_filled,"%d",NULL);

printk("\n   dpcm ");
	MYPRINT(dpcm,buffer_size_in_bytes,"%d",NULL);
	MYPRINT(dpcm,period_size_in_bytes,"%d",NULL);
	MYPRINT(dpcm,pcm_bps,"%d",NULL);
	MYPRINT(dpcm,pcm_jiffie,"%d",NULL);
printk("\n        ");
	MYPRINT(dpcm,pcm_irq_pos[0],"%d",NULL);
	MYPRINT(dpcm,pcm_buf_pos[0],"%d",NULL);
	MYPRINT(dpcm,pcm_irq_pos[1],"%d",NULL);
	MYPRINT(dpcm,pcm_buf_pos[1],"%d",NULL);
...
if(runtime->buffer_size%runtime->period_size) printk("%s: warning! buffer_size(%d) is not integer times  period_size(%d)!!\n   the last link-list descriptor totsize is wrong. fix it!\n\n",__FUNCTION__,runtime->buffer_size,runtime->period_size);
if(runtime->period_size>0x1000) printk("%s: period_size(%d) (in frames) is larger than max size that dma can process(4096) in one time\n    this is not implemented. fix it!\n\n",__FUNCTION__,runtime->period_size);
	...
	if((!dpcm->desc||!dpcm->desc_size_in_bytes||!(dpcm->desc_size_in_bytes==tmp))){
		...
		if(dpcm->desc) {
			pci_free_consistent(NULL, dpcm->desc_size_in_bytes, dpcm->desc, dpcm->desc_p);
			dpcm->desc=dpcm->desc_p=NULL;
			dpcm->desc_size_in_bytes=0;
		}
		dpcm->desc_size_in_bytes=tmp;
		dpcm->desc = pci_alloc_consistent(NULL, dpcm->desc_size_in_bytes, &dpcm->desc_p);
		if(!dpcm->desc){
