...
static irqreturn_t str8131_int28vbus_irq_handler(int this_irq, void *dev_id, struct pt_regs *regs)
{
...
static int __init str8131_int28vbus_inthandler_init(void)
{
	int ret;
printk("%s: \n",__FUNCTION__);
	if(PWRMGT_USB_DEVICE_POWERMGT_REG&0x1)
	...
	//str8131_set_interrupt_trigger (INTC_GPIO_EXTERNAL_INT_BIT_INDEX,INTC_LEVEL_TRIGGER,INTC_ACTIVE_HIGH);
	if ((ret=request_irq(INTC_USB_DEVICE_VBUS_BIT_INDEX, str8131_int28vbus_irq_handler, 0, "vbus", NULL))){
...
static void __exit str8131_int28vbus_inthandler_exit(void) 
{ 
	printk("%s: \n",__FUNCTION__);
	lock_kernel();
	free_irq(INTC_USB_DEVICE_VBUS_BIT_INDEX, NULL);
...
module_init(str8131_int28vbus_inthandler_init);
module_exit(str8131_int28vbus_inthandler_exit);
