...
//Ricky Cao: Below is for apply the patch to fix Slingcatcher
// ArnonM: Disable HW checksums because eth packets with non-zero trailer bytes produce TCP errors from this HW
// (Performance will be hurt only for packets generated on the device, and NAT-routed packets)
/// #define STAR_GSW_TX_HW_CHECKSUM
/// #define STAR_GSW_RX_HW_CHECKSUM
//Ricky Cao: Above is for apply the patch to fix Slingcatcher
...
//Ricky Cao: Below is for apply the patch to fix Slingcatcher
// ArnonM: Restored delayed interrupts because disabling can cause TCP packets to get stuck in system, instead of getting RX interrupt
#define STAR_GSW_DELAYED_INTERRUPT
//Ricky Cao: Above is for apply the patch to fix Slingcatcher

#define MAX_PEND_INT_CNT	0x06
//Ricky Cao: Below is for apply the patch to fix Slingcatcher
/*ArnonM: Lowering timer delay from 0x20. 0x04 (80usecs) gave 1.365Mbps avg for Wifi-5.2.3, 0x12 (360usecs): 1.126Mbps 
 * 0x01 (20 usecs) is the lowest we can go with this feature enabled, and means that the feature is not really used.
 * It is only left because disabling entirely causes WLS-1929: TCP packets are stuck until a retransmission occurs. */
//Ricky Cao: Above is for apply the patch to fix Slingcatcher
#define MAX_PEND_TIME		0x01
...
//#define STAR_GSW_MAX_RFD_NUM	128
//Jacky.Yang 9-Jul-2009, Follow Cavium suggestion to modifye it.
