...
static int
str8131_change_mtu(struct net_device *netdev, int new_mtu)
{
...
#ifdef FREE_TX_SKB_MULTI
#if 0
	HAL_NIC_READ_TXSD(tssd_current);
...
//	if (skb_free_count >= MAX_TX_SKB_FREE_NUM) {
		int count = 0;
//		for (i = 0; i < MAX_TX_SKB_FREE_NUM; i++) {
...
//	}
	...
	for (f = 0; f < (nr_frags + 1); f++) {
		...
		cur_index++;
		if (cur_index == STAR_NIC_MAX_TFD_NUM) {
			cur_index = 0;
		}
	}
	...
	txring.cur_index++;
	if (txring.cur_index == STAR_NIC_MAX_TFD_NUM) {
		txring.cur_index = 0;
	}
...
int inline str8131_nic_suspend(pm_message_t state)
{
...
#ifdef CONFIG_STAR_JUMBO
	netdev->change_mtu = &str8131_change_mtu;
#endif
...
{
	star_nic_proc_entry = create_proc_entry("str8131/nic", S_IFREG | S_IRUGO, NULL);
...
static int agc_th[2] = {0x18, 0x28};
static int cuv[3][3] = {
	{1, 1, 4},
	{1, 1, 0},
	{1, 1, -4}};
static u32 phy_mon_timer;
