...
//Tim Wang, blinking system LED
#define SYSTEM_LED_GPIO  2
#define GPIO_ON  PIN_TRIG_HIGH
#define GPIO_OFF PIN_TRIG_LOW
#define BLINKING_DELAY 1
int sys_blinking_flag = 1;

void stop_blinking_SYS_LED(int stop_value)
{
	if (stop_value > 0) {
		sys_blinking_flag = 0;
	}
...
*/
static irqreturn_t str9100_gpio_irq_handler(int this_irq, void *dev_id, struct pt_regs *regs)
{
	int i;
	u32 gpio_intr;
...
//Tim Wang, let gpioDevice call it to stop blinking system LED
EXPORT_SYMBOL(stop_blinking_SYS_LED);
...
void blinking_system_led(unsigned long gpio_onoff) 
{
	int led_on_off;
		
	if (sys_blinking_flag == 1) {
		led_on_off=gpio_onoff;
		str9100_gpio_out_bit(led_on_off, SYSTEM_LED_GPIO);	
		if (led_on_off == GPIO_ON) {
			led_on_off = GPIO_OFF;
		} else {
			led_on_off = GPIO_ON;
		}		
		init_timer(&sysled_timer);
		sysled_timer.expires = SECS_TO_JIFFIES(BLINKING_DELAY)+jiffies; // 150 msec total.
		sysled_timer.data = led_on_off;
		sysled_timer.function = blinking_system_led;
	 	add_timer(&sysled_timer);
	} else {
		str9100_gpio_out_bit(GPIO_OFF, SYSTEM_LED_GPIO);	
	}
...
int __init str9100_gpio_init(void)
{
	...
	//Tim Wang, blinking the system LED.
	str9100_gpio_out_bit(GPIO_ON, SYSTEM_LED_GPIO);	
	init_timer(&sysled_timer);
	sysled_timer.expires = SECS_TO_JIFFIES(BLINKING_DELAY)+jiffies; // 150 msec total.
	sysled_timer.data = GPIO_OFF;
	sysled_timer.function = blinking_system_led;
  	add_timer(&sysled_timer);
