...
#if defined(CONFIG_ARCH_STR9100)
...
extern int fast_bridge_forward_skb(struct sk_buff *skb);
#endif
...
int netif_rx(struct sk_buff *skb)
{
    ...
    if (star9100_shnat_pci_fp_forward_skb_ptr && star9100_shnat_pci_fp_forward_skb_ptr(skb) == 0) {
        return NET_RX_SUCCESS;
    }
#endif

#ifdef CONFIG_FAST_BRIDGE
	if (fast_bridge_forward_skb(skb) == 0) {
		return NET_RX_SUCCESS;
	}
#endif
		...
		void (*setup)(struct net_device *))
{
	...
	p = str9100_dspad_alloc(alloc_size);
#else
...
#if defined(CONFIG_ARCH_STR9100) && defined(CONFIG_CPU_DSPAD_ENABLE)
		str9100_dspad_free((char *)dev - dev->padded);
#else
