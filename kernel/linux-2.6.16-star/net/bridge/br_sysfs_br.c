...
/* ArnonM: Added parameter to control setting the bridge loop breaking feature */

static ssize_t show_loop_breaking(struct class_device *cd, char *buf)
{
	return sprintf(buf, "%lu\n", to_bridge(cd)->loop_breaking);
}

static void set_loop_breaking(struct net_bridge *br, unsigned long val)
{
	br->loop_breaking = val;
}

static ssize_t store_loop_breaking(struct class_device *cd, const char *buf,
				size_t len)
{
	return store_bridge_parm(cd, buf, len, set_loop_breaking);
}

static CLASS_DEVICE_ATTR(loop_breaking, S_IRUGO | S_IWUSR, show_loop_breaking,
			 store_loop_breaking);
...
static struct attribute *bridge_attrs[] = {
	...
	&class_device_attr_loop_breaking.attr,
	NULL
};
